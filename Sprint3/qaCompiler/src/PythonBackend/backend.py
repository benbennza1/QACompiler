...
def createFirstStepFunction(functionName, nextStepNumber):
    ...
    if (functionName != ""):
        ...
        addToGraph(functionName,"step" + str(nextStepNumber))
    addNewLineToLine()      
...
def doFunction (stmt, stepNumber, tabOffSet = 0):
    ...
    if (checkFunctionName(functionId)):
        ...
        if (times_variant == "0"):
            ...
            addToLine ("err=" + functionId + "_step(browser,loop=True)", 1 + tabOffSet)
...
def goToStep(stmt, stepNumber, tabOffSet = 0, functionName=""):
    ...
    addToGraph("step"+stepNumber,returnFunctionName + goStepNumber)
...
def clickButton (stmt, stepNumber, tabOffSet= 0 ):
    ...
    addToLine ("if d>10000:", 2 + tabOffSet)
    addToLine ('return "' + stepNumber +'"', 3 + tabOffSet)     
...
def stmtHandler(stmt, stepNumber, tabOffSet = 0, functionName=""):
    ...
    if (variant == "0"):
        ...
        loopStep(stmt, stepNumber, tabOffSet)       
    elif (variant == "1"):
        #DO:Do ID:ID IntegerTime:IntegerTime
        doFunction(stmt, stepNumber, tabOffSet)        
    elif (variant == "2"):
        #GO:Go TO:To URL:URL
...
# handles result
def resultHandler(result, stepNumber):
...
def stepLastReturn (num, functionName = ""):          
    ...
    if (num != ""):
        addToGraph(returnFunctionName+str(int(num)-1),"step" + num)
    ...
    for step in setOfSteps:
        ...
        if (stepLineVariant == "0"): 
            ...
            if (stmt is None and stepLine.find(STR_EXIT) is not None):  
                ...
                resultHandler(result, numLiteral)
...
def printWarnings():
    ...
    for func in functionNames :
            if (func not in  usedFunctions):
                addToLine("print(\"WARNING: " + func +" is never used\")",1)
                print ("WARNING: " + func +" is never used")
...
def compileXMLtoPython (xmlPath = 'outputs14.html.xml', outputFileName = 'compiledCode.py',preprocessorFileName = ''):
